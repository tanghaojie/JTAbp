...
{
    public class JTUserManager<TRole, TUser> : UserManager<TUser>, IDomainService
        ...
        where TUser : JTUser<TUser>
    {
        ...
        protected JTUserStore<TRole, TUser> UserStore { get; }
        ...
        {
            return UserStore.FindByNameOrEmail(userNameOrEmailAddress);
        }
        ...
        {
            return UserStore.FindAll(login);
        }
        ...
        {
            return UserStore.Find(tenantId, login);
        }
        ...
        {
            return UserStore.FindByNameOrEmail(tenantId, userNameOrEmailAddress);
        }
        ...
        {
            var user = UserStore.FindById(userId.ToString());
            if (user == null)
            {
                throw new AbpException("There is no user with id: " + userId);
            }
        ...
        public override async Task<IdentityResult> UpdateAsync(TUser user)
        {
            ...
            if (user.UserName != JTUserBase.AdminUserName)
            {
                if ((await GetOldUserNameAsync(user.Id)) == JTUserBase.AdminUserName)
                {
                    throw new UserFriendlyException(string.Format(L("CanNotRenameAdminUser"), JTUserBase.AdminUserName));
                }
            }
        ...
        public override async Task<IdentityResult> DeleteAsync(TUser user)
        {
            if (user.UserName == JTUserBase.AdminUserName)
            {
                throw new UserFriendlyException(string.Format(L("CanNotDeleteAdminUser"), JTUserBase.AdminUserName));
            }
        ...
        {
            await UserStore.UserRepository.EnsureCollectionLoadedAsync(user, u => u.Roles);
        ...
        public virtual async Task InitializeOptionsAsync(int? tenantId)
        {
            ...
            //Lockout
            Options.Lockout.AllowedForNewUsers = await IsTrueAsync(JTSettingNames.UserManagement.UserLockOut.IsEnabled, tenantId);
            Options.Lockout.DefaultLockoutTimeSpan = TimeSpan.FromSeconds(await GetSettingValueAsync<int>(JTSettingNames.UserManagement.UserLockOut.DefaultAccountLockoutSeconds, tenantId));
            Options.Lockout.MaxFailedAccessAttempts = await GetSettingValueAsync<int>(JTSettingNames.UserManagement.UserLockOut.MaxFailedAccessAttemptsBeforeLockout, tenantId);

            //Password complexity
            Options.Password.RequireDigit = await GetSettingValueAsync<bool>(JTSettingNames.UserManagement.PasswordComplexity.RequireDigit, tenantId);
            Options.Password.RequireLowercase = await GetSettingValueAsync<bool>(JTSettingNames.UserManagement.PasswordComplexity.RequireLowercase, tenantId);
            Options.Password.RequireNonAlphanumeric = await GetSettingValueAsync<bool>(JTSettingNames.UserManagement.PasswordComplexity.RequireNonAlphanumeric, tenantId);
            Options.Password.RequireUppercase = await GetSettingValueAsync<bool>(JTSettingNames.UserManagement.PasswordComplexity.RequireUppercase, tenantId);
            Options.Password.RequiredLength = await GetSettingValueAsync<int>(JTSettingNames.UserManagement.PasswordComplexity.RequiredLength, tenantId);
        }
        ...
        public virtual void InitializeOptions(int? tenantId)
        {
            ...
            //Lockout
            Options.Lockout.AllowedForNewUsers = IsTrue(JTSettingNames.UserManagement.UserLockOut.IsEnabled, tenantId);
            Options.Lockout.DefaultLockoutTimeSpan = TimeSpan.FromSeconds(GetSettingValue<int>(JTSettingNames.UserManagement.UserLockOut.DefaultAccountLockoutSeconds, tenantId));
            Options.Lockout.MaxFailedAccessAttempts = GetSettingValue<int>(JTSettingNames.UserManagement.UserLockOut.MaxFailedAccessAttemptsBeforeLockout, tenantId);

            //Password complexity
            Options.Password.RequireDigit = GetSettingValue<bool>(JTSettingNames.UserManagement.PasswordComplexity.RequireDigit, tenantId);
            Options.Password.RequireLowercase = GetSettingValue<bool>(JTSettingNames.UserManagement.PasswordComplexity.RequireLowercase, tenantId);
            Options.Password.RequireNonAlphanumeric = GetSettingValue<bool>(JTSettingNames.UserManagement.PasswordComplexity.RequireNonAlphanumeric, tenantId);
            Options.Password.RequireUppercase = GetSettingValue<bool>(JTSettingNames.UserManagement.PasswordComplexity.RequireUppercase, tenantId);
            Options.Password.RequiredLength = GetSettingValue<int>(JTSettingNames.UserManagement.PasswordComplexity.RequiredLength, tenantId);
        }
        ...
        {
            return UserStore.GetUserNameFromDatabase(userId);
        }

        private async Task<JTUserPermissionCacheItem> GetUserPermissionCacheItemAsync(long userId)
        {
            ...
            return await _cacheManager.GetUserPermissionCache().GetAsync(cacheKey, async () =>
            {
                ...
                var newCacheItem = new JTUserPermissionCacheItem(userId);
        ...
        private JTUserPermissionCacheItem GetUserPermissionCacheItem(long userId)
        {
            var cacheKey = userId + "@" + (GetCurrentTenantId() ?? 0);
            return _cacheManager.GetUserPermissionCache().Get(cacheKey, () =>
            {
                ...
                foreach (var roleName in UserStore.GetRoles(user))
                {
        ...
        public virtual void AddTokenValidityKey(
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            UserStore.AddTokenValidityKey(user, tokenValidityKey, expireDate, cancellationToken);
        }
        ...
        public virtual bool IsTokenValidityKeyValid(
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return UserStore.IsTokenValidityKeyValid(user, tokenValidityKey, cancellationToken);
        }
        ...
        public virtual void RemoveTokenValidityKey(
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            UserStore.RemoveTokenValidityKey(user, tokenValidityKey, cancellationToken);
        }
        ...
        public bool IsLockedOut(string userId)
        {
            ...
            var lockoutEndDateUtc = UserStore.GetLockoutEndDate(user);
        ...
        {
            UserStore.ResetAccessFailedCount(user);
        }
    }
}
